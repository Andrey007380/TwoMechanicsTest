...
//				Debug.Log(ind + " not collide grass " + name);
			...
			Vector3 forward = Vector3.down;//tsf.forward;
			for (int y = 0; y < height; y++)
			{
				for (int x = 0; x < width; x++)
				{
					...
					//Debug.Log(CalcFalloff(dir.magnitude, 1f));
					...
					if (pixel.b > 0.5f && Vector2.Angle(dir, characterForward) < 180)
					{
						...
						pixels[x + y * width] = new Color(1F, 1F, pressure, 1);//new Color(dir.x, dir.y, pressure, 1);
					}
				}
			}
